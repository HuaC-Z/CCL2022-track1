...
def get_eval_metrics(conf_set, total):
        print(f"tp:{len(conf_set['tp'])}\ttn:{len(conf_set['tn'])}\tfp:{len(conf_set['fp'])}\tfc:{len(conf_set['fn'])}")

        if (len(conf_set["tp"]) + len(conf_set["fn"])) == 0 or len(conf_set["tp"]) + len(conf_set["fp"]) ==0:
            metrics = OrderedDict({"Accuracy": 0, "Precision": 0, "Recall": 0, "F1": 0})
            return metrics
...
def get_char_metrics(hyp_path, gold_path, only_detectioin=False):
        ...
        metrics = OrderedDict({"Detection": detect_metrics}) 
        
        if not only_detectioin:
            correct_metrics = get_eval_metrics(correct_conf_dict, len(hyp_map))
            metrics["Correction"] = correct_metrics
