...
def metric_file(pred_path, targ_path, only_detectioin=False):
...
def sent_metric_detect(preds, targs):
    ...
    for pred_item, targ_item in zip(preds, targs):
        ...
        if pred != [] and len(pred) == len(targ) and all(p[0] == t[0] for p, t in zip(pred, targ)):
            tp += 1
    print(f"sentence--->tp:{tp}\ttarg_p:{targ_p}\tpred_p:{pred_p}")
    if tp == 0 or targ_p == 0 or pred_p ==0:
        results = OrderedDict({
        'Accuracy': 0,
        'Precision': 0,
        'Recall': 0,
...
def get_sent_metrics(hyp, gold, only_detectioin=False):
    metrics = metric_file(
        pred_path=hyp,
        targ_path=gold,
    )
