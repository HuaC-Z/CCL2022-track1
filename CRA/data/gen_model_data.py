...
def get_yinxiangsi_info(word, pos = 0, cdx = 0, ci_update_ratio = 0.05, is_nosie = False):
	...
	if random.random() < ci_update_ratio:
		...
		if select_cdx_list:
			...
			for select_cdx in select_cdx_list:
				...
				if not is_chinese_string(words[cdx]): continue

				if select_cdx < 0:

					pos = select_err_info["start_pos"]
	...
	for cdx, word in enumerate(words):
		...
		if not pre_word_err and random_value < per_word_ratio:
			...
			# 类型是随机替换
				...
				s_select, err_info = get_random_info(word,
					pos = pos, cdx = cdx)
