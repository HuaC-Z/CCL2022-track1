...
def train(args, model, tokenizer):
	""" Train the model """
	cached_file_dir = args.data_dir # "data/cached"
	if args.local_rank == -1 or args.local_rank == args.calc_rank:

		train_features = read_file_2_features(os.path.join(cached_file_dir, args.train_file), tokenizer = tokenizer,
			is_only_sent = False, split_str = "\t", re_ratio = 0.5, desc = "train")

		valid_features = read_file_2_features(os.path.join(cached_file_dir, args.valid_file), tokenizer = tokenizer,
	...
	tr_loss = 0.0
	best_f1 = 0; no_add = 0; total_num = 0; global_step = 0
	model.zero_grad()
	...
	best_f1 = 0.0; best_model = None; no_add = 0
	for _ in range(int(args.num_train_epochs)):

		if _ > 0 and (args.local_rank == -1 or args.local_rank == args.calc_rank):
			train_features = read_file_2_features("data/train.txt", tokenizer = tokenizer,
				is_only_sent = False, split_str = " ||| ", re_ratio = 0.5, desc = "train")
...
if __name__ == '__main__':
	...
	parser.add_argument("--data_dir", default="./data/cached", type=str,
						help="data type")
