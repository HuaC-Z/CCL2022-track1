...
class Trainer:
    ...
    def __init__(self, config):
        ...
        # self.valid_dataloader = init_dataloader(config.train_path, config, "dev", self.tokenizer)
    ...
    def __forward_prop(self, dataloader, back_prop=True):
        ...
        for batch in tqdm(dataloader):
            ...
            # loss, logits, logits_tag = self.model(**batch)
            loss, logits_tag = self.model(**batch)
            # outputs = torch.argmax(logits, dim=-1)
            outputs_ = torch.argmax(logits_tag, dim=-1)
            # print(outputs, outputs_)
            # for outputs_i in outputs:
            #     collected_outputs.append(outputs_i)
            for outputs_i_ in outputs_:
                collected_outputs_.append(outputs_i_)
            loss_sum += loss.item()
            if back_prop:
                loss.backward()
                self.optimizer.step()
        ...
        # return epoch_loss, collected_outputs, collected_outputs_
        return epoch_loss, collected_outputs_
    ...
    def train(self):
        no_improve = 0
        print('epoch_num:', self.config.num_epochs)
        for epoch in range(1, self.config.num_epochs + 1):
            print('cur_epoch:', epoch)
            self.model.train()
            # train_loss, _, t = self.__forward_prop(self.train_dataloader, back_prop=True)
            train_loss, _ = self.__forward_prop(self.train_dataloader, back_prop=True)
            self.model.eval()
            with torch.no_grad():
                # valid_loss, valid_output, valid_output_ = self.__forward_prop(self.valid_dataloader, back_prop=False)
            ...
            get_best_score(self.best_score, self.best_epoch, epoch,
                            char_metrics["Detection"]["F1"], sent_metrics["Detection"]["F1"])
            if max(self.best_epoch.values()) == epoch:
                self.__save_ckpt(epoch)
            # except Exception as e:
            #     print(f"Decoded files cannot be evaluated.{e}")
            #     pass
